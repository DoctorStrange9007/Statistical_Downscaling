...
class LSTMLayer(tf.keras.layers.Layer):
    ...
    def call(self, S, X):
        ...
        Z = self.trans1(
            tf.add(tf.add(tf.matmul(X, self.Uz), tf.matmul(S, self.Wz)), self.bz)
        )
        G = self.trans1(
            tf.add(tf.add(tf.matmul(X, self.Ug), tf.matmul(S, self.Wg)), self.bg)
        )
        R = self.trans1(
            tf.add(tf.add(tf.matmul(X, self.Ur), tf.matmul(S, self.Wr)), self.br)
        )

        H = self.trans2(
            tf.add(
                tf.add(tf.matmul(X, self.Uh), tf.matmul(tf.multiply(S, R), self.Wh)),
                self.bh,
            )
        )
        ...
        S_new = tf.add(
            tf.multiply(tf.subtract(tf.ones_like(G), G), H), tf.multiply(Z, S)
        )
...
class DGMNet(tf.keras.Model):
    ...
    # constructor/initializer function (automatically called when new instance of class is created)
    def __init__(self, settings, final_trans=None):
        """
        Args:
            layer_width:
        ...
        self.input_dim = settings["general"]["d"]
        self.layer_width = settings["pre_trained"]["model"]["nodes_per_layer"]
        self.learning_rate = settings["pre_trained"]["model"]["learning_rate"]

        # define initial layer as fully connected
        # NOTE: to account for time inputs we use input_dim+1 as the input dimensionality
        self.initial_layer = DenseLayer(
            self.layer_width, self.input_dim + 1, transformation="tanh"
        )

        # define intermediate LSTM layers
        self.n_layers = settings["pre_trained"]["model"]["num_layers"]
        self.LSTMLayerList = []

        for _ in range(self.n_layers):
            self.LSTMLayerList.append(LSTMLayer(self.layer_width, self.input_dim + 1))

        # define final layer as fully connected with a single output (function value)
        self.final_layer = DenseLayer(1, self.layer_width, transformation=final_trans)
